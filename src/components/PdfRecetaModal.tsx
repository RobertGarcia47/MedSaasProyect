...
import { generarRecetaPdf, PLANTILLAS, type RecetaPdfData } from '../lib/pdf';

/**
 * Modal de receta: previsualiza el PDF (con la plantilla guardada de la clínica)
 * y permite descargar o imprimir. La plantilla se elige en Configuración, no aquí.
 */
export function PdfRecetaModal({ open, data, onClose, toast }: {
  open: boolean;
  data: RecetaPdfData | null;
  onClose: () => void;
  toast?: (m: string) => void;
}) {
  ...
  // Genera el PDF al abrir / cambiar de receta.
  useEffect(() => {
    if (!open || !data) return;
    let cancelado = false;
    setError(null);
  ...
  const nombreArchivo = `Receta${data?.folio ? ' ' + data.folio.replace(/\s+/g, '') : ''}.pdf`;

  const obtenerBlob = async (): Promise<Blob> => {
    if (blobRef.current) return blobRef.current;
    const blob = await generarRecetaPdf(data!);
    blobRef.current = blob;
    return blob;
  };
    ...
    <Dialog open={open} onClose={onClose} width={920} style={{ height: '95vh', maxHeight: '95vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', borderBottom: '1px solid var(--outline-variant)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="title-l" style={{ fontWeight: 700, fontSize: 17 }}>Receta médica</div>
      ...
      <div style={{ flex: 1, minWidth: 0, background: 'var(--surface-container-lowest)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        {error ? (
          ...
          <>
            {previewUrl && (
              <iframe title="Vista previa de la receta" src={previewUrl}
